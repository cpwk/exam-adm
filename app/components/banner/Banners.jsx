...
export default class Banners extends React.Component {
    ...
    loadProps = () => {
        App.api('oms/banner/bannerTypes').then((bannerTypes) => {
    ...
    loadData = () => {
        ...
        let {type} = this.state;
        App.api('oms/banner/banners', {bannerQo: JSON.stringify({type})}).then((banners) => {
    ...
    remove = (id, index) => {
        ...
        Modal.confirm({
            title: `确认删除操作?`,
            onOk() {
                App.api('oms/banner/remove', {id}).then(() => {
    ...
    render() {
        ...
        return <div className="common-list">
            ...
            <BreadcrumbCustom first={CTYPE.link.pc.txt}/>
